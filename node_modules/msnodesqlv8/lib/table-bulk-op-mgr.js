class BulkPromises {
  ...
  insert (rows) {
    return this.op(rows, (rows, cb) => this.bulk.insertRows(rows, cb))
  }

  delete (rows) {
    return this.op(rows, (rows, cb) => this.bulk.deleteRows(rows, cb))
  }

  select (rows) {
    return this.op(rows, (rows, cb) => this.bulk.selectRows(rows, cb))
  }

  update (rows) {
    return this.op(rows, (rows, cb) => this.bulk.updateRows(rows, cb))
  }
}
...
class TableBulkOpMgr {
  ...
  batchIterator (sql, rows, iterate) {
    return Promise
